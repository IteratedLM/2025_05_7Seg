...
fg = RGB{Float32}(1.0, 0.0, 0.0)
bg = RGB{Float32}(0.8, 0.8, 0.8)

function save_img_color(img, filename, scale; 
                  bg=bg,
                  fg=fg)
    # 1) Upsample if requested
    if scale != 1
        img = kron(img, ones(Float32, scale, scale))
    end
